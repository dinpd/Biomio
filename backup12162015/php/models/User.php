...
class User {
  ...
  public static function add_profile($first_name, $last_name, $email, $type, $ip, $extention) {
  	...
  	$result = $pdo->prepare("INSERT INTO Emails (profileId, email, `primary`, extention) VALUES (:profileId, :email, :primary, :extention)");
...
// User Services
  ...
  public static function save_api_keys($profileId, $pub, $priv) {
	include ('connect.php');
	$result = $pdo->prepare("INSERT INTO ProviderKeys (profileId, public_key, private_key) VALUES (:profileId, :pub, :priv)");
	$result->execute(array('profileId'=>$profileId, 'pub'=>$pub, 'priv'=>$priv));
	return $result;
  }

  public static function delete_api_key($profileId, $key) {
	include ('connect.php');
	$result = $pdo->prepare("DELETE FROM ProviderKeys WHERE profileId = :profileId AND public_key = :key");
	$result->execute(array('profileId'=>$profileId, 'key'=>$key));
	return $result;
  }
