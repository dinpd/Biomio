...
console.log(userId + ' ' + timertimer );

    if($("log_in_email")=="admin@admin.com") {

    }
    if (userId != "" && window.timertimer == 0) {
        	timertimer = Math.floor((Math.random() * 6) + 3); 
		window.timertimer = timertimer * 1000;
		setTimeout(function(){ 
			console.log('here2');
			$('body').addClass('match');
               	 stopSnapshots();
	...
	// This is 2014 demo. Currently it is not connected to a real application and intended for demonstration of the interface only.
	/*$.ajax({
            type: "POST",
...
function render_form() {
    $.ajax({
        url: "./biomio_implementation/biomio-verify.html",
        method: 'GET',
        async: false,
