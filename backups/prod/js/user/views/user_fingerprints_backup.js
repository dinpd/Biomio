...
App.Views.UserFingerprints = Backbone.View.extend({
    ...
    loadUser: function() {
        if (this.userId != "") {
            $.ajax({
                url: 'http://ec2-54-187-197-187.us-west-2.compute.amazonaws.com/api/users/' + this.userId
            }).done(function(data) {
    ...
    processDaraURL: function(image) {
        ...
        if (this.samples < this.maxSamples) {
            $.ajax({
                ...
                $.ajax({
                    type: "PUT",
                    url: 'http://ec2-54-187-197-187.us-west-2.compute.amazonaws.com/api/users/' + this.userId + '/fingerprint/' + sampleId
                }).done(function(data) {
