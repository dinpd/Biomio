...
function loadUser() {
    ...
    if ($('.biomio-id').not('.biomio-input').length > 0 && $('.biomio-id').not('.biomio-input').val()!='')
        ...
        url = 'http://ec2-54-187-197-187.us-west-2.compute.amazonaws.com/api/users/' + $('.biomio-name').not('.biomio-input').val();
        ...
        url = 'http://ec2-54-187-197-187.us-west-2.compute.amazonaws.com/api/users/' + $('.biomio-name.biomio-input').val();
    ...
    if (biomioId != "") {
        $.ajax({
            ...
            if (score > 50) {
                $('body').addClass('match');
                stopSnapshots();
                biomio_success (score);
            } else if (score > 40) {
                stopSnapshots();
                $('.biomio-video').addClass('hide');
                $('.biomio-audio').removeClass('hide');
                initAudio();
            }
            continueSnapshots();
...
$(document).on('click touchend', ".biomio_overlay", function () {
    stopSnapshots(); 
    $('.biomio_body').addClass('hide');
    $('.biomio_overlay').addClass('hide');
    $('.biomio_user_form').addClass('hide');

    $('.biomio-video').removeClass('hide');
    $('.biomio-audio').addClass('hide');
...
$(document).on('click touchend', ".biomio-submit", function (event) { 
    ...
    if ($('.biomio-name.biomio-input').val()=='' &&
        ...
        $('.biomio-error').html("Please, enter your BIOMIO Username, Email, or ID");
    } else {
        loadUser();
        startCamera();
        startSnapshots();
...
function render_button() {
    $.ajax({
        url: "http://biom.io/biomio_login/audio/button-template.html",
        method: 'GET',
        async: false,
...
function render_form() {
    $.ajax({
        url: "http://biom.io/biomio_login/audio/biomio-verify.html",
        method: 'GET',
        async: false,
        success: function(data) {
